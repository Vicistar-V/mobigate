...
import { nominationFeeStructures, mobifaceNominationConfig, type CommunityFeePolicy } from "@/data/nominationFeesData";
...
function CommunityFeePolicyCard() {
  ...
  const [locked, setLocked] = useState(true);
  const [policy, setPolicy] = useState<CommunityFeePolicy>(mobifaceNominationConfig.communityFeePolicy);
  ...
  return (
    <LockableSetting
      ...
      onLockedChange={(v) => {
        ...
        // Saving
        mobifaceNominationConfig.communityFeePolicy = draft;
        setPolicy(draft);
        setLocked(true);
        toast({
