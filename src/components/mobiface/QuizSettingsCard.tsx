...
export function QuizSettingsCard() {
  ...
  const [bankMobiface, setBankMobiface] = useState(platformQuestionBankDistribution.mobifacePercent);
  ...
  const bankTotal = bankMobiface + bankMerchantOwn + bankOtherMerchants;
  ...
  const hasChanges =
    ...
    bankMobiface !== platformQuestionBankDistribution.mobifacePercent ||
  ...
  const handleSave = async () => {
    ...
    setQuestionBankDistribution(bankMobiface, bankMerchantOwn, bankOtherMerchants);

    toast({
      title: "Quiz Settings Updated",
      description: `All settings saved. Question Bank: ${bankMobiface}% Central / ${bankMerchantOwn}% Merchant / ${bankOtherMerchants}% Others. Changes apply platform-wide.`,
    });
    setIsSaving(false);
  };
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold">{bankMobiface}%</span>
              </div>
              <Slider
                value={[bankMobiface]}
                onValueChange={(v) => setBankMobiface(v[0])}
          ...
          <div className="flex items-start gap-2 p-2.5 bg-muted/30 rounded-lg">
            <AlertCircle className="h-3.5 w-3.5 text-muted-foreground shrink-0 mt-0.5" />
            <p className="text-xs text-muted-foreground">
              Controls how questions are sourced during gameplay. Central = Mobiface-curated questions, Merchant = the quiz host's custom questions, Others = questions from other merchants' banks.
            </p>
          </div>
        </div>
