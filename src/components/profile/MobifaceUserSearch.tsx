...
interface MobifaceUserSearchProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Single-select callback (backward compat) */
  onSelect: (user: MobifaceUser) => void;
  selectedUserId?: string;
  /** Enable multi-select mode */
  multiSelect?: boolean;
  /** Multi-select callback */
  onSelectMultiple?: (users: MobifaceUser[]) => void;
  /** Pre-selected user IDs for multi-select */
  selectedUserIds?: string[];
}

export const MobifaceUserSearch = ({
  open,
  onOpenChange,
  onSelect,
  selectedUserId,
  ...
  const handleSingleSelect = () => {
    const user = mockMobifaceUsers.find(u => u.id === tempSelected);
    if (user) {
      onSelect(user);
      onOpenChange(false);
    }
  ...
  const handleMultiConfirm = () => {
    const users = mockMobifaceUsers.filter(u => multiSelected.has(u.id));
    if (users.length > 0) {
