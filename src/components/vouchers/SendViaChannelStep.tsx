...
export function SendViaChannelStep({ channel, remainingMobi, onBack, onSendComplete }: SendViaChannelStepProps) {
  ...
  // Single-channel state (email, whatsApp, sms)
  ...
  // Multi-recipient state (mobiChat)
  ...
  const [sentResults, setSentResults] = useState<Array<{ user: MobigateUser; amount: number; pin: string }>>([]);

  const totalAllocated = selectedUsers.reduce((s, r) => s + r.amount, 0);
  ...
  const isValidRecipient = () => {
    ...
    if (channel === "mobiChat") return selectedUsers.length > 0;
    if (channel === "whatsApp" || channel === "sms") return recipient.replace(/\D/g, "").length >= 10;
    return false;
  };

  // Single-channel
  const canSend = channel !== "mobiChat"
    ? (isValidRecipient() && amount > 0 && amount <= remainingMobi)
    : false;

  // Multi-channel
  const canSendMulti = channel === "mobiChat" && selectedUsers.length > 0 && selectedUsers.every(r => r.amount > 0) && totalAllocated <= remainingMobi;
  ...
  // ─── Send handlers ───
  const handleSend = () => {
    if (channel === "mobiChat") return handleSendMulti();
    if (!canSend) return;
    setSending(true);
  ...
  const handleSendMulti = () => {
    if (!canSendMulti) return;
    setSending(true);
    const results = selectedUsers.map(r => ({
      user: r.user,
      amount: r.amount,
      pin: generatePin(),
    }));
  ...
  const handleCopyPin = (pin?: string) => {
    const pinToCopy = pin || generatedPin;
    navigator.clipboard.writeText(pinToCopy).catch(() => {});
    setPinCopied(true);
  ...
  const handleContinue = () => {
    if (channel === "mobiChat" && sentResults.length > 0) {
      const totalSent = sentResults.reduce((s, r) => s + r.amount, 0);
      const names = sentResults.map(r => r.user.name).join(", ");
  ...
  // ─── SENDING LOADING ───
  if (sending) {
    const sendingTo = channel === "mobiChat"
      ? `${selectedUsers.length} recipient${selectedUsers.length > 1 ? "s" : ""}`
      : recipient;
    const sendingAmount = channel === "mobiChat" ? totalAllocated : amount;
    ...
    return (
      <div className="bg-background min-h-screen flex flex-col items-center justify-center px-6">
        ...
        <p className="text-xs text-muted-foreground">
          M{formatNum(sendingAmount)} to {sendingTo}
        </p>
  ...
  if (sent) {
      ...
      return (
        <div className="bg-background min-h-screen pb-28">
          <div className="flex flex-col items-center justify-center px-6 pt-12 pb-6">
            ...
            <h2 className="text-xl font-bold text-foreground mb-1 animate-fade-in">PINs Sent!</h2>
            ...
            <p className="text-sm text-muted-foreground mb-4 animate-fade-in">
              Sent to {sentResults.length} recipient{sentResults.length > 1 ? "s" : ""} via Mobi-Chat
            </p>
          </div>

          {/* Per-recipient breakdown */}
          <div className="px-4 space-y-3 mb-6">
            {sentResults.map(({ user, amount: amt, pin }) => (
              <div key={user.id} className="rounded-xl border border-border/50 bg-card p-3 animate-fade-in">
    ...
    // Single-recipient success (email, whatsApp, sms)
    return (
      <div className="bg-background min-h-screen flex flex-col items-center justify-center px-6">
        <div className="relative w-24 h-24 mb-6 animate-scale-in">
          <div className="absolute inset-0 rounded-full bg-emerald-500/20 animate-ping" style={{ animationDuration: "2s" }} />
        ...
        <p className="text-sm text-muted-foreground mb-4 animate-fade-in">
          via {config.recipientLabel} to {recipient}
        </p>
  ...
  // ─── MOBI-CHAT MULTI-RECIPIENT FORM ───
  if (channel === "mobiChat") {
    return (
      <div className="bg-background min-h-screen pb-28">
        {/* Sticky header */}
        <div className="sticky top-16 z-20 bg-background/95 backdrop-blur-sm border-b border-border/50">
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Send PINs to multiple users</p>
            </div>
          </div>
          {/* Balance banner */}
        ...
        <div className="px-4 pt-4 space-y-4">
          ...
          >
            ...
            <div className="flex-1 text-left">
              ...
              <p className="text-xs text-muted-foreground">
                {selectedUsers.length > 0
                  ? "Tap to add or remove users"
                  : "Choose Mobigate users to send to"}
              </p>
            </div>
          ...
          {/* Selected recipients with amounts */}
          {selectedUsers.length > 0 && (
            <div className="space-y-3">
              ...
              {selectedUsers.map(({ user, amount: userAmount }) => {
                  ...
                  <div key={user.id} className="rounded-xl border border-border/50 bg-card p-3">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <button
                        ...
                        className="h-7 w-7 rounded-full bg-destructive/10 flex items-center justify-center touch-manipulation active:scale-90 shrink-0"
                      >
                    ...
                    <div className="flex gap-1.5 mb-2 flex-wrap">
                      {userQuickAmounts.map(amt => (
                        <button
                          key={amt}
                          onClick={() => setUserAmount(user.id, amt)}
                          className={`h-7 px-3 rounded-lg text-[10px] font-bold touch-manipulation active:scale-90 transition-all ${
                            userAmount === amt
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setUserAmount(user.id, Math.max(0, userAmount - 100))}
                        className="h-9 w-9 rounded-full bg-muted flex items-center justify-center active:scale-90 touch-manipulation shrink-0"
                      >
                      ...
                      <div className="flex-1 flex items-center gap-2 rounded-xl border border-border/50 bg-background px-3 h-9">
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          value={userAmount || ""}
                          onChange={e => setUserAmount(user.id, parseInt(e.target.value) || 0)}
                          placeholder="0"
                      ...
                      <button
                        onClick={() => setUserAmount(user.id, userAmount + 100)}
                        className="h-9 w-9 rounded-full bg-primary flex items-center justify-center active:scale-90 touch-manipulation shrink-0"
                      >
        ...
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-card/95 backdrop-blur-sm border-t border-border/50 px-4 py-3 safe-area-bottom">
          ...
          >
            ...
            Send M{formatNum(totalAllocated)} to {selectedUsers.length} recipient{selectedUsers.length !== 1 ? "s" : ""}
          </Button>
        </div>
      </div>
    );
  ...
  // ─── SINGLE-RECIPIENT FORM (email, whatsApp, sms) ───
  return (
    <div className="bg-background min-h-screen pb-28">
      {/* Sticky header */}
      <div className="sticky top-16 z-20 bg-background/95 backdrop-blur-sm border-b border-border/50">
