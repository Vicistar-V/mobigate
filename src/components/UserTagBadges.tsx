...
import { Gavel, Flag, Calendar, ChevronRight, EyeOff, Eye, User } from "lucide-react";
import { getVisibleTags, getAllTags, hideTag, unhideTag, type UserTag, type TagType } from "@/data/userTags";
...
export function UserTagBadges({ userId, showAdminControls = false, className = "" }: UserTagBadgesProps) {
  ...
  return (
    <>
      <div className={`flex items-center gap-1.5 flex-wrap ${className}`}>
        {visibleTags.map((tag) => (
          ...
          >
            <Badge
              variant="outline"
              className={`text-xs h-6 px-2 whitespace-nowrap flex items-center gap-1 cursor-pointer active:scale-95 transition-transform ${
                tag.type === "penalised"
      ...
      {/* History Drawer */}
      <Drawer open={!!selectedTag} onOpenChange={(open) => { if (!open) setSelectedTag(null); }}>
