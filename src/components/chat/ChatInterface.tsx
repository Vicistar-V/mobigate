...
export const ChatInterface = ({
  ...
  return <div className="flex-1 flex flex-col h-full bg-white">
      ...
      {isSelectionMode && <div className="px-4 py-3 bg-[#00a884] flex items-center justify-between text-white flex-shrink-0">
          ...
          <Button variant="ghost" size="sm" className="text-white hover:bg-white/20" onClick={onDeleteSelectedMessages}>
            Delete
          </Button>
        </div>}
      ...
      {!isSelectionMode && <div className="w-full border-b bg-[#f9f9f9] border-[#e9edef]">
          ...
          <div className="w-full py-3 px-2 flex items-center justify-start border-b border-[#e9edef] relative bg-[#f0e699]">
          ...
          {/* User Info Row */}
          <div className="px-4 py-[10px] flex items-center bg-[#25d366]/20 border-b-[6px] border-blue-500">
      ...
      <div className="flex-1 p-5 overflow-y-auto flex flex-col bg-[#E5DDD5]" style={{
      backgroundImage: 'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAARMAAAC3CAMAAAAGjUrGAAAAA1BMVEXm5+i+5p7XAAAAR0lEQVR4nO3BMQEAAADCoPVPbQ0PoAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAADeDcYqAAE0I2HfAAAAAElFTkSuQmCC")'
        ...
        return <MessageContextMenu key={message.id} message={message} isOwnMessage={isCurrentUser} onReply={handleReply} onEdit={handleEdit} onDelete={handleDelete} onReact={onReactToMessage}>
              <div className={cn("flex gap-2 mb-4 max-w-[85%] sm:max-w-[70%] animate-in fade-in slide-in-from-bottom-2 duration-300", isCurrentUser ? "self-end" : "self-start")}>
                {isSelectionMode && <div className="flex items-center">
                    <Checkbox checked={isSelected} onCheckedChange={() => onToggleSelectMessage(message.id)} className="h-5 w-5" />
                  </div>}
                <div className={cn("px-2 sm:px-3 py-2 rounded-lg shadow-sm relative group", isCurrentUser ? "bg-[#d9fdd3] rounded-br-none" : "bg-white rounded-bl-none", isSelected && "ring-2 ring-[#00a884]")} onClick={() => isSelectionMode && onToggleSelectMessage(message.id)}>
                  {/* Reply Preview */}
                  {message.replyTo && <div className="mb-2 p-2 bg-black/5 rounded border-l-2 border-[#00a884]">
                  ...
                  {/* Attachments */}
                  {message.attachments && message.attachments.length > 0 && <div className="mb-2 space-y-2">
                      {message.attachments.map((attachment, idx) => <div key={idx}>
                          {attachment.type === 'video' ? <div className="relative rounded-lg overflow-hidden max-w-xs">
                              <video src={attachment.url} controls className="w-full rounded-lg" style={{
                      maxHeight: '300px'
                    }}>
                                Your browser does not support video playback.
                              </video>
                            </div> : attachment.type === 'image' ? <img src={attachment.url} alt={attachment.name} className="max-w-[240px] sm:max-w-[300px] w-full rounded-lg cursor-pointer" onClick={() => window.open(attachment.url, '_blank')} /> : attachment.type === 'gift' && attachment.giftData ? <div className="p-4 rounded-lg bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-950/20 dark:to-purple-950/20 border-2 border-pink-200 dark:border-pink-800">
                              <div className="flex items-center gap-3">
                                {attachment.giftData.icon && <span className="text-4xl">{attachment.giftData.icon}</span>}
                                {!attachment.giftData.icon && <Gift className="h-10 w-10 text-pink-500" />}
                                <div className="flex-1">
                                  ...
                                  {attachment.giftData.category && <Badge className="mt-1 text-base bg-pink-200 dark:bg-pink-900 text-pink-800 dark:text-pink-200 border-0">
                                      {attachment.giftData.category}
                                    </Badge>}
                                </div>
                              </div>
                            </div> : attachment.type === 'audio' ? <div className="flex items-center gap-3 p-3 rounded-lg bg-[#f0f2f5] dark:bg-[#2a2a2a] min-w-[200px] max-w-[300px]">
                              <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-[#00a884] hover:bg-[#00a884]/90 text-white shrink-0" onClick={() => handleAudioPlayback(message.id, attachment.url)}>
                                {playingAudioId === message.id ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5 ml-0.5" />}
                              </Button>
                              
                              <div className="flex-1 flex items-center gap-0.5 h-8">
                                {[...Array(15)].map((_, i) => <div key={i} className="w-1 bg-[#00a884] rounded-full" style={{
                        ...
                        opacity: playingAudioId === message.id ? 1 : 0.5
                              ...
                              <audio id={`audio-${message.id}`} src={attachment.url} onEnded={() => setPlayingAudioId(null)} className="hidden" />
                            </div> : <a href={attachment.url} download={attachment.name} className="flex items-center gap-2 p-2 bg-black/5 rounded hover:bg-black/10">
                  ...
                  {/* Message Content */}
                  {message.content && !message.attachments?.some(att => att.type === 'audio') && <p className="text-base text-[#111b21] break-words mb-1">{message.content}</p>}

                  {/* Reactions */}
                  {message.reactions && message.reactions.length > 0 && <div className="flex gap-1 mt-1 mb-1">
                      {Array.from(new Set(message.reactions.map(r => r.emoji))).map(emoji => {
                  const count = message.reactions!.filter(r => r.emoji === emoji).length;
                  return <span key={emoji} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-black/5 rounded-full text-base">
                             {emoji} {count > 1 && count}
                           </span>;
                })}
                    </div>}
                   ...
                   <div className="flex items-center gap-1 mt-1">
                     ...
                     {message.isEdited && <span className="text-base text-[#667781] italic">• edited</span>}
                     {isCurrentUser && <span className="text-base text-[#667781] ml-1">
                  ...
                  {!isSelectionMode && <div className="absolute -left-8 top-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => {
                e.stopPropagation();
          ...
          {/* Typing Indicator */}
          {isTyping && <div className="flex self-start mb-4">
              <div className="bg-white rounded-lg rounded-bl-none shadow-sm px-3 py-2 flex items-center gap-1">
                <span className="w-2 h-2 bg-[#9E9E9E] rounded-full animate-bounce" style={{
            animationDelay: "0ms"
          }} />
                <span className="w-2 h-2 bg-[#9E9E9E] rounded-full animate-bounce" style={{
      ...
      {/* Input Area */}
      <ChatInput onSendMessage={onSendMessage} disabled={isTyping} replyTo={replyTo} onCancelReply={() => setReplyTo(null)} recipientName={conversation.user.name} onStartQuiz={onStartQuiz} />

      {/* Edit Message Dialog */}
      <EditMessageDialog open={editingMessage !== null} onOpenChange={open => !open && setEditingMessage(null)} initialContent={editingMessage?.content || ""} onSave={handleSaveEdit} />
    </div>;
};