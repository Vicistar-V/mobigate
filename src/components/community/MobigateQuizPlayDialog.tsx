...
import { X, Clock, Trophy, AlertTriangle, CheckCircle, XCircle, Wallet, Globe, Flame, Zap } from "lucide-react";
...
export function MobigateQuizPlayDialog({ open, onOpenChange, quiz, playerWalletBalance, onGameComplete }: MobigateQuizPlayDialogProps) {
  ...
  const moveToNextQuestion = () => {
    ...
    else { setCurrentQuestionIndex(prev => prev + 1); setSelectedAnswer(null); setShowResult(false); setTimeRemaining(quiz?.timeLimitPerQuestion || 25); setGameState("playing"); }
  };

  const startGame = () => {
    if (!quiz) return;
    if (playerWalletBalance < quiz.stakeAmount) { toast({ title: "Insufficient Balance", variant: "destructive" }); return; }
    toast({ title: "🔥 Game Started!", description: `${quiz.currency} ${quiz.stakeAmount.toLocaleString()} deducted. Let's go!` });
  ...
  const handleGameCompleteClick = () => {
    ...
    const { percentage, amount } = calculateMobigateWinnings(finalCorrect, quiz.winningAmount);
  ...
  if (!quiz) return null;
  const finalResult = gameState === "game_over" ? calculateMobigateWinnings(answers.filter(a => a.isCorrect).length, quiz.winningAmount) : null;
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-r from-amber-500 to-orange-500 border-b p-4 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-semibold text-sm line-clamp-1 flex items-center gap-2">{quiz.title} <Flame className="h-3 w-3" /></h2>
                <p className="text-xs text-amber-100">{gameState === "pre_game" ? "Ready to compete" : gameState === "game_over" ? "Game Complete" : `Question ${currentQuestionIndex + 1} of ${questions.length}`}</p>
              </div>
            </div>
          ...
          {gameState !== "pre_game" && (<div className="mt-3"><Progress value={(totalAnswered / questions.length) * 100} className="h-2 bg-amber-400 [&>div]:bg-white" /><div className="flex justify-between mt-1 text-xs text-amber-100"><span>{questionsCorrect} correct</span><span>{totalAnswered}/{questions.length}</span></div></div>)}
        </div>

        <div className="flex-1 overflow-y-auto p-4">
          {gameState === "pre_game" && (
            <div className="space-y-4">
              <Card className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border-amber-200"><CardContent className="p-4 space-y-4">
                <div className="grid grid-cols-2 gap-3"><div className="text-center p-3 bg-white dark:bg-background rounded-lg border border-red-200"><p className="text-xs text-muted-foreground">Stake</p><p className="font-bold text-lg text-red-600">{quiz.currency} {quiz.stakeAmount.toLocaleString()}</p></div><div className="text-center p-3 bg-white dark:bg-background rounded-lg border border-green-200"><p className="text-xs text-muted-foreground">Win Amount</p><p className="font-bold text-lg text-green-600">{quiz.currency} {quiz.winningAmount.toLocaleString()}</p></div></div>
                {quiz.prizePool && <div className="p-3 bg-gradient-to-r from-amber-100 to-orange-100 rounded-lg text-center"><p className="text-xs text-amber-600">🏆 Prize Pool</p><p className="font-bold text-xl text-amber-700">{quiz.currency} {quiz.prizePool.toLocaleString()}</p></div>}
              ...
              <Card><CardContent className="p-4 space-y-3"><h3 className="font-semibold text-sm text-amber-700">Winning Structure</h3><div className="space-y-2 text-sm"><div className="flex items-center justify-between p-2 bg-green-50 rounded-lg"><span className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-500" />10/10</span><span className="font-semibold text-green-600">100% Win</span></div><div className="flex items-center justify-between p-2 bg-yellow-50 rounded-lg"><span className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-yellow-500" />8-9/10</span><span className="font-semibold text-yellow-600">50% Win</span></div><div className="flex items-center justify-between p-2 bg-red-50 rounded-lg"><span className="flex items-center gap-2"><XCircle className="h-4 w-4 text-red-500" />&lt;8/10</span><span className="font-semibold text-red-600">No Prize</span></div></div></CardContent></Card>
              <Card className={cn("border-2", playerWalletBalance >= quiz.stakeAmount ? "border-green-300 bg-green-50" : "border-red-300 bg-red-50")}><CardContent className="p-4"><div className="flex items-center gap-3"><Wallet className={cn("h-5 w-5", playerWalletBalance >= quiz.stakeAmount ? "text-green-500" : "text-red-500")} /><div className="flex-1"><p className="text-sm font-medium">Your Wallet</p><p className="text-lg font-bold">{quiz.currency} {playerWalletBalance.toLocaleString()}</p></div><Badge variant={playerWalletBalance >= quiz.stakeAmount ? "outline" : "destructive"} className={playerWalletBalance >= quiz.stakeAmount ? "bg-green-100 text-green-600" : ""}>{playerWalletBalance >= quiz.stakeAmount ? "Sufficient" : "Insufficient"}</Badge></div></CardContent></Card>
              <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm"><AlertTriangle className="h-4 w-4 text-amber-600 mt-0.5 shrink-0" /><p className="text-muted-foreground">Compete with players worldwide! Stake deducted on start.</p></div>
            </div>
          )}

          {(gameState === "playing" || gameState === "question_result") && currentQuestion && (
            <div className="space-y-4">
              <div className="flex items-center justify-center gap-2"><Clock className={cn("h-5 w-5", timeRemaining <= 10 ? "text-red-500 animate-pulse" : "text-amber-600")} /><span className={cn("text-2xl font-bold tabular-nums", timeRemaining <= 10 ? "text-red-500" : "")}>{timeRemaining}s</span></div>
              <Progress value={(timeRemaining / quiz.timeLimitPerQuestion) * 100} className={cn("h-2", timeRemaining <= 10 && "[&>div]:bg-red-500")} />
              <Card className="bg-amber-50 dark:bg-amber-950/30 border-amber-200"><CardContent className="p-4"><p className="text-base font-medium leading-relaxed">{currentQuestion.question}</p></CardContent></Card>
              <div className="grid grid-cols-2 gap-2">
                {currentQuestion.options.map((option, index) => {
                  const label = MOBIGATE_ANSWER_LABELS[index];
                  const isSelected = selectedAnswer === index;
                  const isCorrect = index === currentQuestion.correctAnswer;
                  const showCorrect = showResult && isCorrect;
                  const showWrong = showResult && isSelected && !isCorrect;
                  return (<button key={index} onClick={() => handleAnswerSelect(index)} disabled={showResult} className={cn("relative p-3 rounded-lg border-2 text-left transition-all hover:border-amber-400", isSelected && !showResult && "border-amber-500 bg-amber-50", showCorrect && "border-green-500 bg-green-50", showWrong && "border-red-500 bg-red-50", !isSelected && !showCorrect && !showWrong && "border-gray-200")}><div className="flex items-start gap-2"><span className={cn("flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold shrink-0", isSelected && !showResult && "bg-amber-500 text-white", showCorrect && "bg-green-500 text-white", showWrong && "bg-red-500 text-white", !isSelected && !showCorrect && !showWrong && "bg-gray-100")}>{label}</span><span className="text-sm leading-tight">{option}</span></div>{showCorrect && <CheckCircle className="absolute top-2 right-2 h-4 w-4 text-green-500" />}{showWrong && <XCircle className="absolute top-2 right-2 h-4 w-4 text-red-500" />}</button>);
                })}
              </div>
              {showResult && (<div className={cn("p-3 rounded-lg text-center font-medium", answers[answers.length - 1]?.isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600")}>{answers[answers.length - 1]?.timeExpired ? "Time expired!" : answers[answers.length - 1]?.isCorrect ? "🔥 Correct!" : `Wrong! Answer: ${MOBIGATE_ANSWER_LABELS[currentQuestion.correctAnswer]}`}</div>)}
            </div>
          )}

          {gameState === "game_over" && finalResult && (
            <div className="space-y-4">
              <Card className={cn("border-2", finalResult.status === "won" && "border-green-500 bg-gradient-to-br from-green-50 to-emerald-50", finalResult.status === "partial_win" && "border-yellow-500 bg-gradient-to-br from-yellow-50 to-amber-50", finalResult.status === "lost" && "border-red-500 bg-red-50")}><CardContent className="p-6 text-center space-y-4"><div className={cn("inline-flex items-center justify-center w-16 h-16 rounded-full", finalResult.status === "won" && "bg-green-100", finalResult.status === "partial_win" && "bg-yellow-100", finalResult.status === "lost" && "bg-red-100")}>{finalResult.status === "won" && <Trophy className="h-8 w-8 text-green-500" />}{finalResult.status === "partial_win" && <Trophy className="h-8 w-8 text-yellow-500" />}{finalResult.status === "lost" && <XCircle className="h-8 w-8 text-red-500" />}</div><div><h2 className="text-xl font-bold">{finalResult.status === "won" ? "🔥 Amazing Win!" : finalResult.status === "partial_win" ? "Good Job!" : "Better Luck!"}</h2><p className="text-muted-foreground mt-1">{answers.filter(a => a.isCorrect).length}/10 correct</p></div>{finalResult.amount > 0 && (<div className="pt-2"><p className="text-sm text-muted-foreground">You won</p><p className="text-3xl font-bold text-amber-600">{quiz.currency} {finalResult.amount.toLocaleString()}</p></div>)}</CardContent></Card>
            </div>
          )}
        </div>

        <div className="sticky bottom-0 z-10 bg-background border-t p-4">
          {gameState === "pre_game" && <Button className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600" size="lg" onClick={startGame} disabled={playerWalletBalance < quiz.stakeAmount}><Zap className="h-4 w-4 mr-2" />{playerWalletBalance >= quiz.stakeAmount ? `Start - Pay ${quiz.currency} ${quiz.stakeAmount.toLocaleString()}` : "Insufficient Balance"}</Button>}
