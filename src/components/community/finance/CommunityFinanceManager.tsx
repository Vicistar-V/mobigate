...
// Category label map for transaction display
const CATEGORY_LABELS: Record<string, string> = {
  advertisement: "📢 Ad Revenue",
  dues:          "💳 Dues",
  levy:          "📋 Levy",
  donation:      "🎁 Donation",
  expense:       "💸 Expense",
...
function txnCategoryLabel(t: any): string {
  if (t.category && CATEGORY_LABELS[t.category]) return CATEGORY_LABELS[t.category];
                  ...
                  <div key={t.id} className="flex items-start gap-3 p-3 rounded-xl border bg-card">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {t.category && (
                          <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full shrink-0">{txnCategoryLabel(t)}</span>
                        )}
                      </div>
