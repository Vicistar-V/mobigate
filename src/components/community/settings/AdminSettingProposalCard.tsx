...
export function AdminSettingProposalCard({
  ...
  const [pendingVote, setPendingVote] = useState<"approve" | "disapprove" | null>(null);
  ...
  const handleVoteClick = (vote: "approve" | "disapprove") => {
    setPendingVote(vote);
  ...
  const handleAuthorizationSuccess = async () => {
    ...
    setLocalVote(pendingVote);
    onVote(proposal.proposalId, pendingVote);

    toast({
      title: pendingVote === "approve" ? "Vote Recorded" : "Disapproval Recorded",
      description: `You have ${pendingVote === "approve" ? "approved" : "disapproved"} "${proposal.settingName}"`,
    });
  ...
  const actionKey = pendingVote === "approve" ? "approve_setting" : "disapprove_setting";
  const actionConfig = getActionConfig("settings", actionKey);

  const actionDetails = actionConfig && pendingVote
    ? renderActionDetails({
        config: actionConfig,
        primaryText: proposal.settingName,
        secondaryText: `${pendingVote === "approve" ? "Approving" : "Disapproving"} setting change`,
        module: "settings",
        additionalInfo: {
          label: `${proposal.currentValue} →`,
  ...
  return (
    <>
          ...
          {hasVoted ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                ...
                onClick={() => handleVoteClick("disapprove")}
                disabled={isVoting}
              >
      ...
      <ModuleAuthorizationDrawer
        ...
        module="settings"
        actionTitle={actionConfig?.title || "Setting Vote Authorization"}
