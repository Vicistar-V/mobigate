...
import { MobifaceQuiz, MOBIFACE_ANSWER_LABELS, calculateMobifaceWinnings } from "@/data/mobifaceQuizData";
import { useToast } from "@/hooks/use-toast";
import { cn } from "@/lib/utils";
import { formatMobiAmount, formatLocalAmount, formatLocalFirst } from "@/lib/mobiCurrencyTranslation";

interface MobifaceQuizPlayDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  quiz: MobifaceQuiz | null;
...
export function MobifaceQuizPlayDialog({ open, onOpenChange, quiz, playerWalletBalance, onGameComplete }: MobifaceQuizPlayDialogProps) {
  ...
  const handleGameCompleteClick = () => {
    ...
    const { percentage, amount } = calculateMobifaceWinnings(finalCorrect, quiz.winningAmount);
  ...
  if (!quiz) return null;
  const finalResult = gameState === "game_over" ? calculateMobifaceWinnings(answers.filter(a => a.isCorrect).length, quiz.winningAmount) : null;
    ...
    <Dialog open={open} onOpenChange={handleExitGame}>
      <DialogContent className="max-w-lg max-h-[95vh] p-0 gap-0">
          ...
          {gameState === "pre_game" && (
            <div className="space-y-4">
              <Card className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border-amber-200"><CardContent className="p-4 space-y-4">
                <div className="grid grid-cols-2 gap-3"><div className="text-center p-3 bg-white dark:bg-background rounded-lg border border-red-200"><p className="text-xs text-muted-foreground">Stake</p><p className="font-bold text-lg text-red-600">{formatMobiAmount(quiz.stakeAmount)}</p><p className="text-xs text-muted-foreground">≈ {formatLocalAmount(quiz.stakeAmount, "NGN")}</p></div><div className="text-center p-3 bg-white dark:bg-background rounded-lg border border-green-200"><p className="text-xs text-muted-foreground">Win Amount</p><p className="font-bold text-lg text-green-600">{formatMobiAmount(quiz.winningAmount)}</p><p className="text-xs text-muted-foreground">≈ {formatLocalAmount(quiz.winningAmount, "NGN")}</p></div></div>
                {quiz.prizePool && <div className="p-3 bg-gradient-to-r from-amber-100 to-orange-100 rounded-lg text-center"><p className="text-xs text-amber-600">🏆 Prize Pool</p><p className="font-bold text-xl text-amber-700">{formatMobiAmount(quiz.prizePool)}</p><p className="text-xs text-muted-foreground">≈ {formatLocalAmount(quiz.prizePool, "NGN")}</p></div>}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {currentQuestion.options.map((option, index) => {
                  const label = MOBIFACE_ANSWER_LABELS[index];
              ...
              {showResult && (<div className={cn("p-3 rounded-lg text-center font-medium", answers[answers.length - 1]?.isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600")}>{answers[answers.length - 1]?.timeExpired ? "Time expired!" : answers[answers.length - 1]?.isCorrect ? "🔥 Correct!" : `Wrong! Answer: ${MOBIFACE_ANSWER_LABELS[currentQuestion.correctAnswer]}`}</div>)}
            </div>
          )}
        ...
        <div className="sticky bottom-0 z-10 bg-background border-t p-4">
          ...
          {gameState === "playing" && <Button className="w-full bg-amber-500 hover:bg-amber-600" size="lg" onClick={handleConfirmAnswer} disabled={selectedAnswer === null}>{selectedAnswer === null ? "Select Answer" : `Confirm ${MOBIFACE_ANSWER_LABELS[selectedAnswer]}`}</Button>}
