...
export function ScholarshipQuizSetupSheet({ open, onOpenChange }: ScholarshipQuizSetupSheetProps) {
  ...
  const handleStart = () => {
    ...
    toast({ title: "🎓 Scholarship Quiz!", description: `${formatLocalAmount(stakeAmount, "NGN")} deducted. Answer 15 questions to win!` });
    // Mock result
    toast({ title: "🎉 You Won!", description: `Scholarship of ${formatLocalAmount(budget, "NGN")} will be credited in ${SCHOLARSHIP_PRIZE_DELAY_DAYS} days.` });
    onOpenChange(false);
  };
