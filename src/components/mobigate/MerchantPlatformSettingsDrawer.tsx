    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader className="px-4">
          <DrawerTitle className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-primary" /> Platform Settings</DrawerTitle>
        </DrawerHeader>
        <DrawerBody className="space-y-5 pb-6">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <NumInput label="Questions/Pack" value={config.questionsPerPack} onChange={v => update({ questionsPerPack: v })} min={5} max={50} />
              <NumInput label="Objective" value={config.objectivePerPack} onChange={v => update({ objectivePerPack: v })} min={1} max={config.questionsPerPack - 1} />
              <NumInput label="Non-Objective" value={config.nonObjectivePerPack} onChange={v => update({ nonObjectivePerPack: v })} min={1} max={config.questionsPerPack - 1} />
            </div>
            <NumInput label="Objective Answer Options" value={config.objectiveOptions} onChange={v => update({ objectiveOptions: v })} min={8} max={10} />
          </section>
          ...
          <section className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Fixed at 20% for Alternative Answer matches</p>
              </div>
              <Badge variant="outline">{config.fairAnswerPercentage}%</Badge>
            </div>
          </section>
        ...
        <DrawerFooter>
          <Button className="h-12" onClick={() => { onSave(config); onOpenChange(false); }}>Save Settings</Button>
