...
export default function SubMerchantVoucherTransactions() {
    ...
    <div className="bg-background min-h-screen pb-6">
      <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm border-b border-border/50">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <button onClick={() => setShowFilters(!showFilters)} className={`h-9 w-9 rounded-full flex items-center justify-center active:scale-90 touch-manipulation ${showFilters ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>
          ...
          <div className="px-4 pb-3 space-y-3 border-t border-border/30 pt-3">
            <div>
              ...
              <div className="flex gap-1.5">
                {([["all","All"],["funding","Funding"],["voucher_generation","Purchase"]] as [FilterType,string][]).map(([val,label]) => (
      ...
      <div className="px-4 pt-3 space-y-2.5">
        {filtered.map(txn => {
          ...
          return (
            <div key={txn.id} onClick={() => txn.batchId ? navigate(`/sub-merchant-voucher-batch/${txn.batchId}`) : undefined}
              ...
              <div className="flex items-start gap-3">
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${isFunding ? "bg-emerald-500/10" : "bg-primary/10"}`}>
                  {isFunding ? <ArrowDownLeft className="h-5 w-5 text-emerald-600" /> : <ArrowUpRight className="h-5 w-5 text-primary" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-bold text-foreground">{isFunding ? "Wallet Funding" : "Voucher Purchase"}</p>
        ...
        {filtered.length === 0 && <div className="text-center py-16"><p className="text-sm text-muted-foreground">No transactions found</p></div>}
      </div>
    </div>
  );
}
