...
import { useWallStatusPosts, useCurrentUserId } from "@/hooks/useWindowData";
...
const Index = () => {
  ...
  const [apiFeedPosts, setApiFeedPosts] = useState<Post[]>([]);

  const fetchFeed = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/posts/feed.php?limit=100`, { credentials: "include" });
      if (!res.ok) return;
      const data = await res.json();
      setApiFeedPosts(data.map((p: any) => ({
        id:                 p.id,
        title:              p.title,
