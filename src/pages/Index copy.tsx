...
const Index = () => {
  ...
  // ── Post actions ─────────────────────────────────────────────────────────────
  const handleSavePost = async (updated: Post) => {
    if (!apiReachable) {
      toast({ title: 'Cannot save', description: 'API is unreachable.', variant: 'destructive' });
      return;
    }
    try {
      const form = new FormData();
      form.append('post_id',  updated.id ?? '');
      form.append('title',    updated.title);
      form.append('subtitle', updated.subtitle ?? '');
      form.append('content',  updated.description ?? '');
      form.append('post_type', updated.type.toLowerCase());
      const r = await apiFetch(`${API_BASE}/posts/update.php`, { method: 'POST', body: form });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      await fetchFeed(contentFilter);
  ...
  const handleDeletePost = async (postId: string) => {
    setFeedData(p => p.filter(x => x.id !== postId));
    setWallData(p => p.filter(x => x.id !== postId));
    if (!apiReachable) return;
    try {
      await apiFetch(`${API_BASE}/posts/delete.php`, {
        method: 'POST',
