...
const CommunityAdminDashboard = () => {
  ...
  // ── Real community info + membership stats ──────────────────────────────
  const [communityInfo, setCommunityInfo] = useState<{ name: string; logo?: string }>({
    name: "Community",
  });
  ...
  const [recentMemberRequests, setRecentMemberRequests] = useState(mockRecentMemberRequests);

  const fetchRealData = async () => {
    if (!communityId) return;
    try {
      // Community name/logo
      const cRes = await fetch(`/api/community/get.php?id=${communityId}`, { credentials: "include" });
      if (cRes.ok) {
        const c = await cRes.json();
        setCommunityInfo({ name: c.name || "Community", logo: c.logoImage || undefined });
      }
    } catch { /* keep previous */ }

    try {
      // Real membership counts + pending applications
      const mRes = await fetch(`/api/community/manage_members.php?community_id=${communityId}`, { credentials: "include" });
      if (mRes.ok) {
        const m = await mRes.json();
        setLiveStats({
          totalMembers: m.totalMembers ?? 0,
          activeMembers: m.totalMembers ?? 0,
          pendingRequests: m.totalPending ?? 0,
          blockedUsers: m.totalBlocked ?? 0,
        });
        if (Array.isArray(m.applications)) {
          setRecentMemberRequests(m.applications.slice(0, 5).map((a: any) => ({
            id: a.id,
            name: a.full_name,
            avatar: a.profile_photo || "/placeholder.svg",
            requestDate: a.applied_at ? new Date(a.applied_at) : new Date(),
  ...
  useEffect(() => { fetchRealData(); }, [communityId]);

  // Merge real membership numbers into the stats object; finance/elections/
  // meetings/content remain mock pending dedicated backend work for those modules.
  const liveAdminStats = { ...mockAdminStats, ...liveStats };
      ...
      <ScrollArea className="h-[calc(100vh-64px)] touch-auto overscroll-contain">
        <div className="px-3 py-3 space-y-3 pb-24">
          ...
          <AdminDashboardHeader
            ...
            communityLogo={communityInfo.logo || communityPerson1}
          ...
          <div className="space-y-2">
            ...
            <AdminMembershipSection
              stats={liveAdminStats}
              recentRequests={recentMemberRequests}
