...
import { useState, useRef, useEffect as useEffectAlias, useCallback } from "react";
...
// profile data now fetched from API via useParams
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Profile = () => {
  ...
  // ── Read user ID from URL: /profile/:id ──────────────────────────────────
  const { id: urlUserId } = useParams<{ id: string }>();

  // ── API profile state ─────────────────────────────────────────────────────
  const [apiProfile, setApiProfile] = useState<any>(null);
  const [apiProfileLoading, setApiProfileLoading] = useState(true);
  const [apiFriendStatus, setApiFriendStatus] = useState<string>("none");
  const [apiUserPosts, setApiUserPosts] = useState<Post[]>([]);

  const API_BASE = (import.meta.env.VITE_API_URL as string) || "/api";

  // Fetch profile whenever URL user ID changes
  const fetchApiProfile = useCallback(async (uid: string) => {
    if (!uid) { setApiProfileLoading(false); return; }
    setApiProfileLoading(true);
    setApiProfile(null);
    setApiUserPosts([]);
    try {
      const res = await fetch(`${API_BASE}/profile/info.php?user_id=${uid}`, { credentials: "include" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      setApiProfile(data);
      setApiFriendStatus(data.friendship_status || "none");
      // If own profile, sync photo with localStorage
      ...
      setApiProfileLoading(false);
    }
  }, [API_BASE]);

  useEffectAlias(() => {
    fetchApiProfile(urlUserId || "");
  }, [urlUserId, fetchApiProfile]);

  // Fetch posts for the viewed user
  useEffectAlias(() => {
    const uid = urlUserId || "";
    if (!uid) return;
    fetch(`${API_BASE}/posts/feed.php?user_id=${uid}&limit=50`, { credentials: "include" })
      .then(r => r.ok ? r.json() : [])
      .then((data: any[]) => setApiUserPosts(data.map((p: any) => ({
      ...
      .catch(() => setApiUserPosts([]));
  }, [urlUserId, API_BASE]);

  // postCreated event — refresh
  useEffectAlias(() => {
    const h = () => fetchApiProfile(urlUserId || "");
    window.addEventListener("postCreated", h);
    return () => window.removeEventListener("postCreated", h);
  }, [urlUserId, fetchApiProfile]);
  
  // currentUserId comes from the URL param or API
  const currentUserId = urlUserId || "";
  ...
  // Posts come from API fetch above; local state for edit/delete
  const [userPosts, setUserPosts] = useState<Post[]>([]);
  // Sync apiUserPosts into userPosts when they load
  useEffectAlias(() => { setUserPosts(apiUserPosts); }, [apiUserPosts]);
  
  // Build userProfile from API data — falls back to empty/own data while loading
  const fmt = (n: number) => n >= 1000 ? (n/1000).toFixed(1)+"k" : String(n);
  const isOwnProfile = apiProfile?.is_own_profile === true;
  const userProfile = {
    name:         apiProfile?.name          || "",
    location:     apiProfile?.location      || "",
    profileImage: isOwnProfile
      ? (apiProfile?.profile_photo || profileImage)
      : (apiProfile?.profile_photo
          ? apiProfile.profile_photo
          : "https://ui-avatars.com/api/?name=" + encodeURIComponent(apiProfile?.name || "U") + "&size=200&background=7c3aed&color=fff"),
    bannerImage:  isOwnProfile
      ? (apiProfile?.banner_image  || bannerImage)
      : (apiProfile?.banner_image  || ""),
    verified:     apiProfile?.is_verified   || false,
    status:       (apiProfile?.is_online ? "Online" : "Offline") as "Online" | "Offline",
    isFriend:     apiFriendStatus === "accepted",
    friendship_status: apiFriendStatus,
    stats: {
      friends:   fmt(apiProfile?.stats?.friends   || 0),
      followers: fmt(apiProfile?.stats?.followers || 0),
      following: fmt(apiProfile?.stats?.following || 0),
      likes:     fmt(apiProfile?.stats?.likes     || 0),
      gifts:     fmt(apiProfile?.stats?.gifts     || 0),
      contents:  fmt(apiProfile?.stats?.contents  || 0),
    }
  };
    ...
    if (apiProfile?.id) {
      try {
        await fetch(`${API_BASE}/friends/block.php`, {
          method: "POST", credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ blocked_id: apiProfile.id }),
        });
      } catch {}
    }
    toast({ title: "User Blocked", description: `You have blocked ${userProfile.name}`, variant: "destructive" });
  };
    ...
    if (apiProfile?.id) {
      try {
        await fetch(`${API_BASE}/friends/follow.php`, {
          method: "POST", credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ following_id: apiProfile.id }),
        });
      } catch { setIsFollowing(was); return; }
    }
    toast({ title: was ? "Unfollowed" : "Following", description: was ? `You unfollowed ${userProfile.name}` : `You are now following ${userProfile.name}` });
  };
  ...
  const handleAddFriend = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/friends/add.php`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipient_id: apiProfile.id }),
      });
      const data = await res.json();
      if (data.success) {
        setApiFriendStatus("pending");
      ...
      <main className="container max-w-4xl mx-auto px-4 py-6 flex-1">
        ...
        <Card className="mb-6 overflow-hidden">
          ...
          <div className="relative h-48 bg-muted group">
            <img 
              src={isOwnProfile ? bannerImage : (userProfile.bannerImage || bannerImage)} 
              alt="Profile Banner"
            ...
            {isOwnProfile && (
              <button
                ...
                onClick={(e) => { e.stopPropagation(); setEditingBanner(true); }}
              >
        ...
        >
          ...
          <TabsContent value="friends">
            <ProfileFriendsTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>
          ...
          <TabsContent value="contents">
            <ProfileContentsTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>

          <TabsContent value="gifts">
            <ProfileGiftsTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>

          <TabsContent value="likes">
            <ProfileLikesTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>

          <TabsContent value="followers">
            <ProfileFollowersTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>

          <TabsContent value="following">
            <ProfileFollowingTab userName={userProfile.name} userId={urlUserId || ""} />
          </TabsContent>
